document.addEventListener('DOMContentLoaded', () => {
        ...
        Object.keys(groupedHistory).forEach(groupName => {
            ...
            if (groupItems.length > 0) {
                ...
                groupItems.forEach(item => {
                    ...
                    textSpan.textContent = item.text.substring(0, 50) + (item.text.length > 50 ? '...' : '');
    ...
    const saveBookmarks = (bookmarks) => {
        ...
        startFlashcardsButton.style.display = getBookmarks().length > 0 ? 'block' : 'none';
    };
...
`;
    ...
    textInput.style.zIndex = 1;
    processedText.style.zIndex = -1;
    renderBookmarks(); // Initial check to set flashcard button visibility
});